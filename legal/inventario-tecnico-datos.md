...
1. **RESUELTO (2026-07-21):** la pantalla de Política de Privacidad afirmaba un borrado automático a 30 días que no existía. Se implementó de verdad: `src/jobs/limpiezaIneCliente.job.js` corre diario (4am), borra del storage (Supabase, bucket `documentos-clientes`) y de la base de datos la foto de INE de cualquier pedido con más de `RETENCION_INE_DIAS` (default 30, configurable por variable de entorno) desde su creación. El texto de la app ahora es verdad.
