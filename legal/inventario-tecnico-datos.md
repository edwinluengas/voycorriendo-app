...
1. **CORREGIDO (2026-07-21):** la pantalla de Política de Privacidad de la app afirmaba que la foto de INE (verificación de edad) "se elimina automáticamente 30 días después", pero ese borrado automático nunca existió en el backend (no hay ningún cron job para esto). Se corrigió el texto en `PoliticaPrivacidadScreen.js` para reflejar la realidad: la foto se conserva mientras sea necesaria para verificación/cumplimiento, y el usuario puede solicitar su eliminación por correo (mismo mecanismo manual que ya existe para el resto de los datos, ver punto 2). **No se implementó borrado automático real** — sigue siendo una tarea pendiente si se quiere ofrecer esa garantía en el futuro.
